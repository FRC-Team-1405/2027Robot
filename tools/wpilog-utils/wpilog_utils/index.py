...
without decoding payloads (except the two DriverStation mode signals).
...
re-walks the file itself. What it keeps:
  ...
  * DriverStation mode spans -- the timeline bands
...
from .modes import compute_mode_spans
from .records import iter_records, parse_control, wpilog_header_end

CYCLE_ENTRY_NAME = '/Timestamp'
_DS_SIGNALS = {'DriverStation/Enabled', 'DriverStation/Autonomous'}
...
class LogIndex:
    ...
    ds_signals: Dict[str, List[Tuple[float, bool]]] = field(repr=False, default_factory=dict)
    ...
    def mode_spans(self) -> List[Tuple[float, float, str]]:
        ...
        Empty when the log has no DriverStation/Enabled data."""
        if not self.ds_signals.get('DriverStation/Enabled'):
            return []
...
def build_index(raw: bytes) -> LogIndex:
    ...
    for entry_id, ts_sec, payload, start, end in iter_records(raw, header_end):
        ...
        if entry_id in ds_ids:
            ...
            if v is not None:
                ds.setdefault(ds_ids[entry_id], []).append((ts_sec, bool(v)))

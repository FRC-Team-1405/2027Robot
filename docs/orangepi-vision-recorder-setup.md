...
**Session folder names.** This Pi has no RTC battery, so its wall clock can reset to a stale build-image date on any power cycle that doesn't reach NTP (typical on a field network). Folders are named `boot<NNNN>-<timestamp>`, where `<NNNN>` is a counter in `RECORDINGS_DIR/.boot_id` that increments once per service start — so folders from the current power-on are always distinguishable from an older one even if the timestamp half of the name is wrong.
